...
class BusBot(QObject):
    ...
    message_end_route_signal = pyqtSignal(str)
    def __init__(self):
        super().__init__()
    ...
    def handle_message(self, message):
        ...
        if message.text == 'Начать смену🚍':
            msg = self.bot.send_message(message.chat.id, "Укажите государственный регистрационный номер автобуса")
            self.bot.register_next_step_handler(msg, self.StartRoute)
    ...
    def EndRoute(self, message):
        number = message.text
        self.message_end_route_signal.emit(number)
    ...
    def StartRouteName(self, message, number):
        name = message.text
        self.message_route_name_signal.emit(name, number)
        self.bot.send_message(message.chat.id, "Информация направлена диспетчеру.\nХорошей смены!")


    def SendMessageToDispatcher(self, message):
        number = message.text
        self.message_signal.emit(number)
        self.bot.send_message(message.chat.id, "Сообщение ортправлено. Ожидайте, скоро с Вами свяжется диспетчер")
